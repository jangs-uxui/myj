...
(function($) {
    ...
    $(function() {
		...
		//사이트맵
		var SiteID = $body.attr('data-siteid');
		
		//사이트맵이동
		$('#header .header_login .login_wrap .sitemap a').on('click', function (event) {
			if(SiteID){
				if(SiteID=='mayor'){
					location.href = 'sub.do?key=1631';
				}
				if(SiteID=='culture'){
					location.href = 'sub.do?key=335';
				}
				if(SiteID=='photo'){
					location.href = 'sub.do?key=359';
				}
				if(SiteID=='health'){
					location.href = 'sub.do?key=498';
				}
				if(SiteID=='agri'){
					location.href = 'sub.do?key=570';
				}
				if(SiteID=='eng'){
					location.href = 'sub.do?key=3443';
				}
				if(SiteID=='jap'){
					location.href = 'sub.do?key=3958';
				}
				if(SiteID=='chi'){
					location.href = 'sub.do?key=4043';
				}
				if(SiteID=='mongol'){
					location.href = 'sub.do?key=4213';
				}
				if(SiteID=='museum'){
					location.href = 'sub.do?key=749';
				}
				if(SiteID=='forest'){
					location.href = 'sub.do?key=923';
				}
				if(SiteID=='nyjse'){
					location.href = 'sub.do?key=4299';
				}
				if(SiteID=='vote'){
					location.href = 'sub.do?key=4481';
				}
				if(SiteID=='reserve'){
					location.href = 'sub.do?key=4251';
				}
				if(SiteID=='thinkj'){
					location.href = 'sub.do?key=4479';
				}
				event.preventDefault();
			}
		});
        ...
        //팝업모아보기
		var $popupLayer = $wrapper.find('.banner_group_layer.popup'),
			popupLayerLength = $popupLayer.length;
        var $public_popup = $('.public_popup_zone'),
			public_popup_length = $public_popup.length;
        $public_popup.each(function(){
            var $this = $(this),
                $popup_slick = $this.find('.public_popup_list'),
				PopupListHtml = $popup_slick.html(),
				$PopupItem = $popup_slick.find('.public_popup_item'),
			...
			if(popupLayerLength==1 && public_popup_length==1){
				//팝업모아보기
				var $popupMoreBtn = $public_popup.find('.more_btn'),
					$popupLayerClose = $popupLayer.find('.close_btn'),
					$popupLayerSlide = $popupLayer.find('.banner_slide_list'),
					$popupCount = $popupLayer.find('.titlebox .count');
				$popupCount.text(PopupItemLength);
				$popupLayerSlide.append(PopupListHtml);
				$popupLayerSlide.slick({
					autoplay : false,
					dots: false,
					slidesToShow: 1,
					slidesToScroll: 1,
					...
					rows: 2,
					infinite: true,
					prevArrow : $('.banner_group_layer.popup .banner_control .control .prev'),
					nextArrow : $('.banner_group_layer.popup .banner_control .control .next'),
					...
					total : $('.banner_group_layer.popup .banner_control .count .total'),
					current : $('.banner_group_layer.popup .banner_control .count .current'),
					customState : function(state) {
						if(state.current < 10) {
							state.current = '0' + state.current;
						}
				...
				$popupMoreBtn.on('click', function (){
					var $closebtn = $('.banner_group_layer.popup .layerbox .close_btn');
					$popupLayer.addClass('active');
					$html.addClass('more_open');
					$popupLayerSlide.slick('setPosition');
					$closebtn.focus();
				});
				$popupLayerClose.on('click', function (){
					$popupLayer.removeClass('active');
					$html.removeClass('more_open');
					$popupMoreBtn.focus();
				});
				$('.banner_group_layer.popup .layerbox .banner_content .banner_control .control .arrow.next').on('focusout', function(event) {
					$('.banner_group_layer.popup .layerbox .close_btn').focus();
				});
			}
        });
    });
